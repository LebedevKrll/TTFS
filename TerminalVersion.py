...
That should be it, have fun, fellow Gambler!"
    ...
    def StartRound(self):
        ...
        if PlayerCards[0][1:] in "1JKQ" and PlayerCards[1][1:] == "A" or PlayerCards[1][1:] in "1JKQ" and PlayerCards[0][1:] == "A":
            return
        ...
        del self.deck[l[0]]
        del self.deck[l[1] - 1]
        print(f"Dealer has {self.dealer[0]} and a closed card\n\
The Player has {self.player[0]} and {self.player[1]}\n\
What's the next move, Captain?")

    def ApplyAction(self, action):
        if action in "Hh" or action == "Hit" or action == "hit":

        elif action in "Pp" or action == "Pat" or action == "pat":
            
        else:


    def Blackjack(self):
        print("Congrats! That's a blackjack! Pure skill, honestly")
    ...
    def LetsGoGambling(self):
        ...
        while True:
            action = input()
            if action not in "Rr$":
                print("Excuse me, could you repeat that?")
            elif action in "Rr":
                print(self.rules)
            else:
                while action != "Stop" or action != "stop":
                    game.StartRound()
                    if not self.dealer:
                        game.Blackjack()
                    else:
                        while action != "Stop" or action != "stop":
                            action = input()
                            if action not in 'HPShps' and action not in ['hit', 'Hit', 'Pat', 'pat', 'split', 'Split']:
                                print("Excuse me, could you repeat that?")
                            else:
